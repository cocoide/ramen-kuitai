  ...
  async ({ shopId }: { shopId: string }) => {
    ...
    return (
      <div className="place-center">
        <Link
          href={`/review?shopId=${shopId}`}
          className="place-center text-primary mr-3"
        >
        ...
        {user != null && shop != null ? (
          <BookmarkButton
            id={shopId}
            name={shop.name}
            Bookmarked={checkIsBookmarked(shopId)}
          />
        ) : (
          <Link href={"/login"} className="bg-white rounded-full place-center">
